...
//retornamos valores de true o false, 0 o 1. Para no esperar infinitamente la respuesta
...
struct RxReturn RxPacket(void) {
	...
	for(t = 0; t < 32; t++){
		respuesta.StatusPacket[t] = 0;
	} 
	for (bCount = 0; bCount < 4; bCount++){
		init_Timer();	//ponemos el contador a 0
		respuesta.TimeOut = 0; //contador de tiempo de la estructura a 0
		Byte_Recibido = 0; 	//No se ha recibido Byte;
		while (!Byte_Recibido){		    //se esta recibiendo datos
			respuesta.TimeOut= TimeOut(5000); //iniciamos el tiempo de respuesta
			if (respuesta.TimeOut) break;	// terminamos el bucle si ha habido Timeout
		}
		if (respuesta.TimeOut) break; //terminamos el bucle si ha habido Timeout
		//si no ha habido Timeout guardamos la respuesta
		respuesta.StatusPacket[bCount] = DatoLeido_UART; //Get_Byte_Leido_UART();
	}
	//desde donde necesitamos leer el dato de respuesta
	bLength = respuesta.StatusPacket[3]+4;
	// datos que nos faltan por leer
	if (!respuesta.TimeOut){
		for (bCount = 4; bCount < bLength ; bCount++){
			...
			Byte_Recibido = 0;
			while (!Byte_Recibido){
				respuesta.TimeOut= TimeOut(5000);
				if (respuesta.TimeOut) break;
			}
			if (respuesta.TimeOut) break;
